...
source_loader, target_loader_unl, class_list, t_length = return_dataset(args)
...
def train():
    ...
    #    lr = 0.01
        ...
        if ste < args.update_pseudo:
            ...
            im_data_t_lab.data.resize_(data_t_lab[0].size()).copy_(data_t_lab[0])
            gt_labels_t_lab.data.resize_(data_t_lab[1].size()).copy_(data_t_lab[1])
            zero_grad_all()
